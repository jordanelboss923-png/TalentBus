    ...
    public partial class FrmDeducciones : Form
    {
        ...
        private void CargarDatos()
        {
            try
            {
                ...
                if (dgvDeducciones.Columns.Contains("Nombre"))
                    dgvDeducciones.Columns["Nombre"].HeaderText = "Nombre";

                if (dgvDeducciones.Columns.Contains("Porcentaje"))
                {
                    dgvDeducciones.Columns["Porcentaje"].HeaderText = "Porcentaje (%)";
                    dgvDeducciones.Columns["Porcentaje"].DefaultCellStyle.Format = "N2";
                }

                if (dgvDeducciones.Columns.Contains("Descripcion"))
                    dgvDeducciones.Columns["Descripcion"].HeaderText = "Descripción";
        ...
        private void BtnGuardar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (_modoEdicion)
                    resultado = _cn.Actualizar(
                        _idSeleccionado, nombre, porcentaje,
                        string.IsNullOrEmpty(descripcion) ? null : descripcion);
                else
                    resultado = _cn.Insertar(
                        nombre, porcentaje,
                        string.IsNullOrEmpty(descripcion) ? null : descripcion);
        ...
        private void Grid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            int id = Convert.ToInt32(dgvDeducciones.Rows[e.RowIndex].Cells["Id"].Value);

            if (dgvDeducciones.Columns[e.ColumnIndex].Name == "btnEditar")
            {
            ...
            else if (dgvDeducciones.Columns[e.ColumnIndex].Name == "btnEliminar")
            {
                string nombre = dgvDeducciones.Rows[e.RowIndex]
        ...
        private void ActualizarPosicionGrid()
        {
            if (pnlFormulario.Visible)
            {
                ...
                dgvDeducciones.Location = new Point(dgvDeducciones.Left, 302);
            }
            else
            {
